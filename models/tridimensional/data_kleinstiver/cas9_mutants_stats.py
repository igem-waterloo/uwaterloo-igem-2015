    ...
    for mutant in mut_lst:
        for mutation in mutant['mutations']:
            ...
            if idx in id_dict.keys():
                id_dict[idx] = id_dict[idx] + 1
            if not idx in id_dict.keys():
                id_dict[idx] = 1
    return id_dict

def ss_freq(pam):
    '''
    Given a pam site returns the secondary structures present in the mutations
    '''
    ...
    for mutant in mut_lst:
        for mutation in mutant['mutations']:
            ...
            if loc in loc_dict.keys():
                loc_dict[loc] = loc_dict[loc] + 1
            if not loc in loc_dict.keys():
                loc_dict[loc] = 1
    return loc_dict
...
def aafin_freq(pam):
    '''
    Given a pam site returns the final amino acid of mutation
    '''
    ...
    for mutant in mut_lst:
        for mutation in mutant['mutations']:
            ...
            if aafin in aafin_dict.keys():
                aafin_dict[aafin] = aafin_dict[aafin] + 1
            if not aafin in aafin_dict.keys():
                aafin_dict[aafin] = 1
    return aafin_dict
...
def aachng_freq(pam):
    '''
    Given a pam site returns the indices and change of amino acids in each mutation in the form
    ...
    for mutant in mut_lst:
        for mutation in mutant['mutations']:
            ...
            if aachng in aachng_dict.keys():
                aachng_dict[aachng] = aachng_dict[aachng] + 1
            if not aachng in aachng_dict.keys():
                aachng_dict[aachng] = 1
    return aachng_dict


def bar_chn_counts(pam):
    """
