...
def pam_In_Range(pamLine,geneRange,mitEfficiencyCutOff=0):
    ...
    # difficult to read
    pamStart = int(words[0]) # Start Position
    pamStop = int(words[1]) # Stop Position
    pamSequence = words[2]      # sgRNA sequence
    pamNGG = words[3]      # PAM sequence
    pamSense = int(words[4]) # plus or minus strand
    pamMITEfficiency = float(words[5]) # MIT efficiency score
    ...
    if all([pamStart > geneRange[0], pamStop < endRange, 'N' not in pamSequence,\
    pamMITEfficiency > mitEfficiencyCutOff]):
        return [pamStart,pamStop,pamSequence,pamNGG,pamSense,pamMITEfficiency];
    else: return "";
...
def pam_Overlap(listOfPams,overlapExtension=0):
    ...
    i=0
    # Overlaps score on efficiency for now, but this should be modified for
    # other scoring, e.g. specifity and location within gene
